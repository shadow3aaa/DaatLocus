...
- Workflow 变更应当视为对可复用 SOP primitive assets 的变更，而不是 prompt 编辑或聊天记忆。
...
- 默认不要把临时 primitive compositions 持久化为新的 primitive specs。

Runtime error correction 改的是全局工具和协议约束。Workflow improvement 改的是某类任务的可复用 SOP primitive specs。
