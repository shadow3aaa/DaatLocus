...
mod tests {
    ...
    #[test]
    fn prompt_payload_uses_responses_json_schema_format() {
        let client = test_client();
        let request = PromptRequest {
            tool_name: "final-answer".to_string(),
            tool_description: "ignored by responses text format".to_string(),
            output_schema: json!({
                "type": "object",
            ...
            system_messages: vec!["system one".to_string(), "system two".to_string()],
            long_term_memory_messages: vec![HistoryMessage::user("remembered")],
            history_messages: vec![],
            current_user_message: "current".to_string(),
            retry_messages: vec![],
        };

        let payload =
            build_prompt_responses_payload(&client, request.clone(), request.output_schema);

        assert_eq!(payload["model"], "gpt-5.4");
        assert_eq!(payload["instructions"], "system one\n\nsystem two");
        assert_eq!(payload["stream"], true);
        assert_eq!(payload["input"][0]["role"], "user");
        assert_eq!(payload["input"][0]["content"][0]["text"], "remembered");
        assert_eq!(payload["input"][1]["content"][0]["text"], "current");
        assert_eq!(payload["text"]["format"]["type"], "json_schema");
        assert_eq!(payload["text"]["format"]["name"], "final-answer");
