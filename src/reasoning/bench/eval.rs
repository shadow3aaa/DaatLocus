...
use super::optimize::{
    ...
    load_or_compile_bench_memory_encoding_tuning, load_or_compile_bench_memory_tuning,
    load_or_compile_bench_terminal_completion_tuning,
};
use super::programs::{
...
pub async fn run_bench_eval_memory_encoding(context: &Context) -> Result<Vec<EvalCaseResult>> {
    let renderer = OpenAIToolRenderer;
    let program = MemoryEncodingProgram;
    let tuning = load_or_compile_bench_memory_encoding_tuning(context).await?;
    Ok(crate::reasoning::eval::run_suite_with_tuning(
        context,
        &renderer,
        &program,
