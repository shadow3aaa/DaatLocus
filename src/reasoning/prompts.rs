...
2. 绝对严禁使用任何交互式全屏终端程序（如 vim, vi, nano, less, top 等）。如果你需要查看文件，请使用 cat, grep, head, tail 或注入的 `spin-edit show`；如果你需要修改文件，优先使用注入的 `spin-edit replace` 或 `spin-edit patch`，不要默认依赖 echo/sed/python 在 shell 里拼接补丁。
