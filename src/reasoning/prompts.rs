...
pub const WORKFLOW_UNIT_WHAT: &str = "A workflow is an evolvable SOP primitive library. Each persisted workflow spec describes one reusable primitive procedure with applicability metadata, preconditions, reusable steps, done criteria, and stable recovery paths.";

pub const WORKFLOW_UNIT_WHEN: &str = "When `<workflow>` shows `bound_workflow_id=<none>`, inspect `<workflow_routing>` in `<afterclaim_context>` as a reusable SOP primitive routing catalog. `primitive_ids` is the full loaded primitive vocabulary for composition awareness; `relevant_primitives` expands only the top task-relevant primitives with capability, input, output, and when metadata. If a primitive fits the claimed work, call `activate_workflow`; if none fits, continue with a normal plan instead of creating a composite workflow merely to proceed. Call `create_workflow` only when the task truly requires authoring a new stable primitive. If the user asks to modify the workflow for a past, existing, or previously discussed task class, treat it as workflow maintenance even when the wording is an ordinary instruction and does not explicitly mention workflows. If a workflow is already bound, do not call `activate_workflow` again just to reaffirm it; continue executing under the current binding.";

pub const WORKFLOW_UNIT_HOW: &str = "A workflow binding is runtime state for the current task and does not rewrite the workflow spec. Persisted workflows are primitives; task-time compositions are ephemeral runtime plans or graphs with artifact handoff and must not be written back as new workflow specs by default. You do not need to manually log daytime workflow outcomes; the runtime writes `WorkflowRunRecord` directly at work-completion boundaries for sleep-time patch or merge. When the user asks or contextually implies that an existing reusable process should change, bind the workflow-editing meta workflow, then use `read_workflow` and `update_workflow`; do not execute or activate the workflow being edited as the current task workflow.";
