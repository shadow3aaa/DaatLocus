...
impl ActionPhaseProgramSpec for ExecuteTaskProgram {
    ...
    fn phase_instructions(&self) -> Vec<&'static str> {
        vec![
            ...
            "不是所有任务都需要 change->verify 全流程；如果任务本质上只是资料搜集、判断、回复或状态整理，在证据足够时可以直接 finish，而不必强行进入文件修改。",
            ...
            "只有当任务明确需要修改工作区文件时，才优先使用注入的 `spin-edit` 工具；单一精确片段替换优先 `spin-edit replace`，多行或多处改动优先 `spin-edit patch`，而不是继续用 shell 命令拼接补丁。",
