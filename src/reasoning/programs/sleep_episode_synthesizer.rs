...
- 成功是常态，只有当成功模式明显可迁移、可复用、能稳定指导未来任务时，才允许 promote_to_l3=true。
- 失败更容易产生 L3，但也必须抽象成未来可执行的策略，不要停留在 case 名称或日志片段。
...
pub struct SleepEpisodeSynthesizerOutput {
    ...
    pub promote_to_l3: bool,
    pub l3_kind: L3EntryKind,
    pub l3_stability: L3EntryStability,
    pub l3_lesson: String,
    pub l3_evidence_summary: String,
    pub l3_retrieval_text: String,
    pub l3_confidence: f64,
    pub reason: String,
}
...
impl Program for SleepEpisodeSynthesizerProgram {
    ...
    fn description(&self) -> &'static str {
        "把单个成功/失败 episode 抽象成可复用策略，并生成 artifacts 与 L3 提议。"
    }
    ...
    fn signature(&self) -> Signature {
        Signature::new("从单个 episode 结果中抽象学习结论。")
            ...
            .input("related memories", "相关 L2/L3 记忆。")
            ...
            .output("promote_to_l3", "是否提升为 L3。")
            .output("l3_kind", "L3 类型。")
            .output("l3_stability", "L3 稳定度。")
            .output("l3_lesson", "L3 一句话经验。")
            .output("l3_evidence_summary", "L3 证据摘要。")
            .output("l3_retrieval_text", "供检索的 L3 文本。")
            .output("l3_confidence", "0 到 1 的置信度。")
            .output("reason", "为什么这样判断。")
            .rule("成功 episode 默认不要 promote_to_l3，除非成功模式明显可迁移且足以稳定指导未来任务。")
...
impl SleepEpisodeSynthesizerProgram {
    ...
    pub fn dataset_ir(
        ...
        ir.push_instruction("如果成功只是常规完成，不必提升到 L3；成功只有在明显可迁移时才 promote。");
