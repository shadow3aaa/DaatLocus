...
export type AgentExpressionStatus =
  | "idle"
  | "thinking"
  | "running"
  | "tooling"
...
const expressionLabelByStatus: Record<AgentExpressionStatus, string> = {
...
const expressionMotionByStatus: Record<AgentExpressionStatus, string> = {
  idle: "opacity-100",
...
function isWorkingStatus(status: AgentExpressionStatus) {
...
export const AgentExpression = memo(function AgentExpression({
  status,
  className,
}: AgentExpressionProps) {
  ...
  return (
    <div
      ...
      data-status={status}
      className={cn(
        "relative flex aspect-[11/15] w-64 items-center justify-center p-5 text-foreground",
        className,
      )}
    >
      <svg
