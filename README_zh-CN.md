...
Release 预编译二进制；发布二进制已经内置 Hindsight sidecar，正常安装不需要
Python、`uv`、PyInstaller，也不会在运行时下载 Python 包。
...
`cargo install daat-locus` 会从 crates.io 源码编译，但不会包含预编译的
Hindsight sidecar。需要完整内置 sidecar 的开发构建，请 clone 仓库后先构建当前
平台的 sidecar：
