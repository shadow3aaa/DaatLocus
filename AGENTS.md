...
- 只基于 workspace `WorkflowRunRecord` 修正 workspace workflow spec
- 产出 workflow patch、workflow merge

builtin workflow 属于基础能力层：

- 由仓库 `workflows/*.md` 经 `build.rs` 编译绑定
- 只读、不可写、不可被 sleep patch 或 merge
- 可被 agent 选择和绑定，但不属于自优化对象
...
- 内置 workflow 位于仓库根目录 `workflows/*.md`，并通过 `build.rs` 编译进程序
- workspace 可演化 workflow 位于 `~/daat-locus-workspace/workflows/*.md`
...
- builtin workflow 不会落入 runtime 可写目录，也不会被优化管线触碰
...
- `create_workflow` 只允许创建 workspace workflow，不允许覆盖 builtin workflow
...
- 去重事件
- 持久化状态
- 加载 builtin workflow 与 workspace workflow specs
